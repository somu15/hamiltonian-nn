...
# -*- coding: utf-8 -*-
"""
Created on Wed Jun 15 22:52:14 2022
...
def get_model(args, baseline):
    ...
    path = "100D_AllenCahn.tar" # .format(args.save_dir, RK4, model_name) # ndpdf-hnn
...
for m in np.arange(0, M + Madapt, 1):
    ...
    traj_len[m] = j
    alpha_req[m] = alpha
    alpha1 = np.minimum(1,np.exp(joint - hamil(np.concatenate((samples[m, :], r_sto), axis=0)))) #  alpha/nalpha # (2**j-2**(j-1)) #  
