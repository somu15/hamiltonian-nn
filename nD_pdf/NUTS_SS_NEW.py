...
# -*- coding: utf-8 -*-
"""
Created on Fri Jun 17 17:02:13 2022
...
def build_tree(theta, r, v, j, epsilon, logw):
    """The main recursion."""
    if (j == 0):
        t_span1 = [0,v * epsilon]
        ...
        rplus = rprime[:]
        logwprime = joint
        sprime = int((logwprime - logw) < 1000) # 1
    else:
        # Recursion: Implicitly build the height j-1 left and right subtrees.
        thetaminus, rminus, thetaplus, rplus, thetaprime, rprime, sprime, logwprime = build_tree(theta, r, v, j - 1, epsilon, logw)
        # No need to keep going if the stopping criteria were met in the first subtree.
        if (sprime == 1):
            if (v == -1):
                thetaminus, rminus, _, _, thetaprime2, rprime2, sprime2, logwprime2 = build_tree(thetaminus, rminus, v, j - 1, epsilon, logw)
            else:
                _, _, thetaplus, rplus, thetaprime2, rprime2, sprime2, logwprime2 = build_tree(thetaplus, rplus, v, j - 1, epsilon, logw)
            # Choose which subtree to propagate a sample up from.
            if (np.random.uniform() < np.min([1,np.exp(logwprime - logsumexp1(logwprime, logwprime2))])): # (np.random.uniform() < (wprime2 / (wprime2 + wprime))):
                thetaprime = thetaprime2[:]
                rprime = rprime2[:]
            # Update the stopping criterion.
            sprime = int(sprime and sprime2 and stop_criterion(thetaminus, thetaplus, rminus, rplus))
            # sprime = int(stop_criterion(thetaminus, thetaplus, rminus, rplus))
            # Update the acceptance probability statistics.
            logwprime = logsumexp1(logwprime, logwprime2)
    return thetaminus, rminus, thetaplus, rplus, thetaprime, rprime, sprime, logwprime
...
def getSample(M, start, eps):
    ...
    for m in np.arange(0, M + Madapt, 1):
        ...
        logw = hamil(y0) # logp - 0.5 * np.dot(r0, r0.T)
        # w = np.exp(-joint)
        ...
        while (s == 1):
            ...
            if (v == -1):
                thetaminus, rminus, _, _, thetaprime, rprime, sprime, logwprime = build_tree(thetaminus, rminus, v, j, epsilon, logw)
            else:
                _, _, thetaplus, rplus, thetaprime, rprime, sprime, logwprime = build_tree(thetaplus, rplus, v, j, epsilon, logw)

            # Use Metropolis-Hastings to decide whether or not to move to a
            # point from the half-tree we just generated.
            alpha = np.min([1,np.exp(logwprime - logw)])
            if (sprime == 1) and (np.random.uniform() < alpha):
                samples[m, :] = thetaprime[:]
                r_sto = rprime
            logw = logsumexp1(logw, logwprime)
...
inp1[0,:,0] = np.ones(2)
y1[0,0] = LS1(inp1[0,:,0].reshape(1,2))
accept1 = np.zeros((Nsub, Nlim))

for ii in np.arange(1,(Nsub),1):
    inp = getSample(1, inp1[ii-1,:,0], eps1)
    inp1[ii,:,0] = inp
    y1[ii,0] = LS1(inp)
    print(ii)
...
for kk in np.arange(0,Nlim,1):
    Pi = len(np.rot90(np.where(y1[:,kk]>np.min([y1_lim[kk],0]))))/(Nsub)
    Pf = Pf * Pi
    Pi_sto[kk] = Pi
    cov_sq = cov_sq + ((1-Pi)/(Pi*Nsub))
cov_req = np.sqrt(cov_sq)

ess_hnn = np.zeros((Nlim,int(args.input_dim/2)))
for ss in np.arange(0,Nlim,1):
    hnn_tf = tf.convert_to_tensor(inp1[:,:,ss])
